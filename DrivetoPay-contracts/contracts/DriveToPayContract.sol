...
contract DriveToPayContract {
    ...
    //stores addresses and business struct
    mapping(address => Business) businessesMap;

    //maps businesses name to their address
    mapping(string => address) nametoBusiness;

    //maps user's license plate to their address
    mapping(string => address) platetoUsers;
    ...
    //registering users
    function RegisterUser(bool business, bool customer)
        public
        returns (string memory Confirmation)
    {
        ...
        if (business == true && customer == false) {
            ...
            return (
                "Congratulations, you are now registered as a Business entity!"
            );
        ...
        //registering as both
            ...
            return (
                "Congratulations, you are now registered as a User and a Business entity!"
            );
        }
    }
    ...
    function payment(
        ...
        string memory NameOfService,
        uint256 amount
    ) external payable OnlyCustomers {
        //checks if the amount send is the right amount for the service requested
        require(
            amount == businessesMap[BusinessAddress].Services[NameOfService],
            "Please send the right amount"
        );
        ...
        businessesMap[BusinessAddress].Balance += amount;
    }

    //finding the price of the service requested
    function findPrice(address BusinessAddress, string memory NameOfService)
        public
        view
        returns (uint256)
    {
    ...
    function withdraw(uint256 amountRequested) public payable OnlyEntity {
        ...
        businessesMap[msg.sender].Balance -= amountRequested;
    }

    //specifying the business
    function createBusiness(
        string memory NameOfB,
        string memory NameOFservice,
        uint256 price
    ) public OnlyEntity {
        ...
        nametoBusiness[NameOfB] = msg.sender;
    }

    //Adding services to the business catalog
    function AddService(string memory NameOfService, uint256 price)
        public
        OnlyEntity
    {
        businessesMap[msg.sender].Services[NameOfService] = price;
    ...
    //user adding their license plate
    function createuserandPlate(string memory licensePlate) public {
        platetoUsers[licensePlate] = msg.sender;
    }

    //returning user's address based on what their license plate is
    function getuserAddress(string memory licensePlate)
        public
        view
        returns (address)
    {
        return platetoUsers[licensePlate];
    }

    //getting balance of the business
    function getBalance() public view returns (uint256) {
        return businessesMap[msg.sender].Balance;
    }

    //getting address of the business
    function getAddress(string memory NameofB) public view returns (address) {
        return nametoBusiness[NameofB];
    }
    ...
    modifier OnlyUsers() {
        require(
            users[msg.sender].business == true ||
                users[msg.sender].customer == true,
